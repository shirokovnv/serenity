...
void main()
{
    ...
    vec4 a = imageLoad(u_input, ivec2(row, col));
    vec4 b = imageLoad(u_input, ivec2(row, col + (u_resolution >> 1)));
    ...
    p <<= 1;
    imageStore(u_output, ivec2(row, q + u_stride * p), fadd);
    imageStore(u_output, ivec2(row, q + u_stride * (p + 1)), fsub);
}
