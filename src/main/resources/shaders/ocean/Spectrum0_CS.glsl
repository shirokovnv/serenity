...
float philips(in vec2 wave_vector){
    ...
    float Lp = V * V / g;  //  (Corrected: no extra squaring)
    float k = length(wave_vector);
    k = max(k, 0.001);  // Small positive minimum to prevent division by zero

    float kv = dot(normalize(wave_vector), normalize(u_wind));

    return clamp(u_amplitude * exp(-1.f / (pow(k * Lp, 2.0))) / pow(k, 4.0f) * kv , 0, 4000);
...
void main(void)
{
    ...
    //vec2 wave_vector = (2.0 * PI * vec2((pixel_coord.x - center_offset.x) / float(u_ocean_size), (pixel_coord.y - center_offset.y) / float(u_ocean_size)));
...
//    vn = clamp(vn, 0, 1000);
    ...
    //imageStore(u_initial_spectrum, pixel_coord, vec4(h, phase, 1));
