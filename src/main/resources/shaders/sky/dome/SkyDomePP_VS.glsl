...
void main()
{
    ...
    // Add params only for the sun halo
    const float haloRadius = 0.2;
    const float haloFalloff = 1.0;

    // Halo intensity
    float haloIntensity = 1.0 - smoothstep(0.0, haloRadius, acos(-cosTheta)); // where halo is
    haloIntensity = pow(haloIntensity, haloFalloff);

    // Apply intensity to sun and halo color
    vec4 haloColor = clamp(I, 0, 1) * (vec4(sunColor.xyz * sunIntensity * intensityFactor, 1));
    haloColor = haloColor * haloIntensity;
    haloColor.w = falloff;

    sunlitColor = haloColor;
}